...
> - R7（GLM）：批次 4 出口 QA-001 Paper E2E 与故障演练（15 用例：T+1/重启/断线/重复回报/对账/Kill Switch/OBS）+ 修复演练暴露的账本幂等缺口
...
## 十、RESEARCH-002：A 股事件与财报新因子（2026-09-01）

- [x] 落地历史板位涨停/接近涨停/放量追板拥挤因子，市场涨跌停广度时序，公告/新闻情绪与龙虎榜净买入的时点安全数据合同。
- [x] 财报可用时间改为实际披露字段优先、报告期 +120 日兜底；新增 ROE 稳定性、盈利加速、利润弹性代理。
- [x] 完成 2023+ 周频因子审计：三个追板拥挤反向因子 IC 约 0.066~0.070；财报候选因 IC/稳定性/覆盖不足全部淘汰。
- [x] 完成组合搜索、首次冻结 OOS、0/1/2 倍成本、固定参数 WFA 与 Admission Gate；制品 `research_audit_20260901_093340_613616_60adaf`。
- [x] 门禁结论为 **FAIL**：2x 成本 CAGR -0.49%、1x Sharpe 0.20、相对沪深300超额 -0.47%；未修改 live allowlist 或正式默认参数。
- [ ] RESEARCH-002 出口仍未满足：补齐全市场实际财报披露/修订、逐日证券状态与指数成分、龙虎榜和公告事件数据；取得新的未见样本后重新通过独立 OOS、成本与容量门禁。

详细报告：`docs/RESEARCH_002_A股事件与财报因子研究_2026-09-01.md`。
