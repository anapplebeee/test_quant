...
from quart.data.universe_history import build_history_from_snapshots, save_history
...
def _retry(fn, retries: int = 3, wait: float = 3.0):
    for attempt in range(1, retries + 1):
        try:
            return fn()
        except Exception as exc:  # noqa: BLE001 - 网络层重试
...
def fetch_st_history(limit: int | None) -> pd.DataFrame:
    ...
    for n, sym in enumerate(todo, 1):
        ...
        try:
            ...
            "symbol": sym, "status": "st",
            "status_effective_from": pd.NaT, "status_effective_to": pd.NaT,
            "observed_at": OBSERVED_AT, "source": "snapshot_fallback",
        }]
        if n % 20 == 0:
            _save_st_cache(done)
...
def fetch_constituents(index_code: str = "000300") -> pd.DataFrame:
    ...
    changes = build_history_from_snapshots(index_code, {pd.Timestamp.now().date().isoformat(): snap})
...
def build_master(listings: pd.DataFrame, delistings: pd.DataFrame, st_rows: pd.DataFrame,
                 suspensions: pd.DataFrame) -> SecurityMaster:
    """装配状态区间行主数据：每符号至少一行 listed；ST/退市为附加状态行。"""
    st_by_sym = {
        sym: g for sym, g in (st_rows.groupby("symbol") if not st_rows.empty else [])
    }
    rows = []

    def add_status(symbol, status, frm, to, rule):
        rows.append({
            "symbol": symbol, "exchange": exchange, "board": board,
            "security_type": "stock", "listed_at": listed, "delisted_at": delisted,
            "status": status, "status_effective_from": frm, "status_effective_to": to,
            "lot_size": 100, "tick_size": 0.01,
            "price_limit_rule": rule, "settlement_rule": "T+1",
        })
    ...
    for _, r in listings.iterrows():
        ...
        if st_g is not None and len(st_g):
            for _, s in st_g.iterrows():
                add_status(symbol, s["status"], s["status_effective_from"],
                           s["status_effective_to"], rule)
...
def main() -> None:
    ...
    if args.st_name_history:
        ...
        st_rows = pd.DataFrame([{
            "symbol": s, "status": "st",
            "status_effective_from": OBSERVED_AT, "status_effective_to": pd.NaT,
            "observed_at": OBSERVED_AT, "source": "snapshot",
    ...
    console.print(f"listed={int((master.table['status']=='listed').sum())} "
