...
- 2026-09-01：`ARCH-001` 已完成。`quart/domain/` 提供统一订单、风控、回报和成交合同，PaperBroker 已改为仅由标准化回报推进状态；旧执行、手动交易和 Broker 模型通过显式适配保留兼容。
- 后续：`DB-001`、`JOB-001` 和 `OMS-001` 负责把该合同持久化，并提供重启恢复、跨进程幂等和真实券商的单写者边界。
