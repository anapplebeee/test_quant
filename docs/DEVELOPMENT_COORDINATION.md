...
| UI-001 | 清除前端直读领域/文件 | 前端 + 后端平台 | 3 | P0 | API-001 | ✅ 2026-09-01 完成：DR-01~06 全部整改，`frontend/` 页面只经 `api/` 层读取——新增 `api/config_api.py`（配置快照+生效参数）与 `data_api`（新鲜度/下一交易日/证券名/ML 分数）、`manual_trading_api`（账户概览/持仓摘要）、`research_api`（因子规格）接口；7 个页面（home、daily_signal、strategy_monitor、risk_management、glossary、factor_research、data_overview）改接 API，新增 13 个 API 层测试 + 前端构建冒烟测试更新，全量测试通过 |
| RESEARCH-002 | 新因子与组合优化 | 量化研究 | 3 | P1 | RESEARCH-001 | ⚠️ 2026-09-02 连板专项仍未通过出口：第一轮已实现财报质量、涨停/追板拥挤、市场广度、龙虎榜与公告事件合同；新增 `limit_streak` 可成交事件研究覆盖首次达到 1~5 板、T+1 开盘拒单和跌停延期退出。预注册的板位 `{2,3,4,5}` × 持有 `{1,2,3,5}` 16 个候选成本前及双基准超额全部为负，主候选二板后持有1日毛收益 -1.19%、1×成本后 -1.49%，判定 FAIL；同时因 qfq、退市/公司行为/PIT 状态证据不完整而正式 BLOCKED，未改任何 allowlist。详见 `docs/RESEARCH_002_A股事件与财报因子研究_2026-09-01.md`、`docs/RESEARCH_008_A股连板策略分析_2026-09-02.md`；待未复权参考价、完整 PIT 状态及 2026-09-03 后新样本再验 |
...
| BROKER-002 | 首个真实券商灰度 | 后端交易 + 交易负责人 | 5 | P1 | QA-001 | ⛔ 2026-09-02 BLOCKED：前置未齐——① `live_allowlist` 为空，尚无任何策略通过准入门禁（`data/meta/admission_status.csv` 无 PASS 记录），没有可灰度报单的实盘策略；② 缺少真实券商凭证/沙箱环境与交易负责人审批。解锁路径：某策略 `scripts/admission_gate.py` 全项通过并进入 `live_allowlist` → 先接查询/资产/成交对账同步 → 再小资金逐笔报单并对账 |
